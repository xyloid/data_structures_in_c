...
- Ring buffer is a data structure which may also be named as circular buffer, circular queue or cyclic buffer. 
- Ring buffer is a single fixed-size buffer which is conceptually connected from end to end. It is a FIFO buffer. Once initialized, a ring buffer's operations such as enqueue and dequeue are constant time. So if an application requires limited memory and buffering stream data at high-frequency, ring buffer is an ideal solution. 
- For example, ring buffer can be used in digital signal processing, such as implementing a finite impulse response (FIR) filter. This is becasue: 
  - FIR filter has fixed window size : Ring buffer has fixed buffer size.
  - FIR filter keeps accepting new sample data and tossing old data : Ring buffer permit overwriting old data.
  - FIR filter may need to work at a high frequency : Accessing to the data sequentially is constant time for the ring buffer.
- Other applications of ring buffer includes:
  - Linux kernel ring buffer, which is used to record messages related to the kernel.
  - Linux network interface has a ring buffer in the main memory in order to deal with network traffic bursts.
