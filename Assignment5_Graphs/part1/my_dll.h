...
typedef struct DLL {
	...
	dll_node_t *head; // head points to the first node in our DLL.
	dll_node_t *tail; // tail points to the last node in our DLL.
} dll_t;
...
int dll_pop_front(dll_t *t) {
	...
	dll_node_t *new_head = (*(*t).head).next;
	free((*t).head);
	(*t).head = new_head;
	// Decrement the count.
	if (--(*t).count == 0) {
...
int dll_pop_back(dll_t *t) {
	...
	dll_node_t *new_tail = (*(*t).tail).previous;
	free((*t).tail);
	(*t).tail = new_tail;
