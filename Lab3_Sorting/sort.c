...
// Output: The index in an array of the minimum value between a range
// [start,stop]
...
void swap(int *a, int *b) {
	...
	// If a and b are not NULL, then proceed.
	if (a && b) {
		int temp = *a;
		(*a) = (*b);
		(*b) = temp;
...
//              This is the start of some 'contiguous block of memory' that we
//              will sort.
...
void sortIntegers(int *array, unsigned int size) {
	...
	if (array) {
		...
		int minimum = 0;
		for (i = 0; i < size; i++) {
			minimum = findMinimum(array, i, size);
			// If minimum is behind element i, then swap them.
			if (minimum > i)
...
void printIntArray(int *array, unsigned int size) {
	if (array == NULL) {
		printf("The array address is NULL.\n");
		return;
	}
	unsigned int
