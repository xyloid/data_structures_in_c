...
1. *Given 'n' nodes, we could use a 'n' rows and 'n' columns matrix to represent their connectivity. Thus the space complexity is O(n<sup>2</sup>).*
2. *It doesn't make sense. If ROWS != COLUMNS, then there must be at least one pair of the nodes having undefined coonnectivity. Thus the adjaceny matrix must be square matrix.*
...
* **Undirected graph:** *The topological sort requires order between nodes. If a graph is undirected, then there is no order. Thus, the topological sort can tell which node comes first and which node comes second. So we can not run topological sort on an undirected graph.*

* **Directed graph with cycles:** *For a directed graph with cycles,there is no order between the nodes on the cycle since each node is another node's descendent and ancestor. So we can't run topological sort on it. To detect cycles, we can add a variable to each node to mark if the node has been visited. Then during the execution of depth first search, if an adjacent node is marked as having been visited, a cycle is found.*
